...
Scene scene4(Ray ray) {
    ...
    scene.tabRay[0] = Translation(scene.tabRay[0], vec3(0., 0., 4.));
    scene.tabScale[0] = vec3(1., 1., 4.);
    scene.tabAngle[0] = vec3(0.5*-iTime, 0., 0.);
    scene.tabRay[0] = Rotation(scene.tabRay[0], scene.tabAngle[0], scene.tabTorus[0].center);
    scene.tabRay[0] = Homothetie(scene.tabRay[0], scene.tabScale[0], scene.tabTorus[0].center);


    scene.nbGoursat = 1;
    scene.tabGoursat[0] = Goursat(vec3(0, 6, 1), 5); 
    scene.tabScale[1] = vec3(1.,2.,1.);
    scene.tabRay[1] = Homothetie(scene.tabRay[1], scene.tabScale[1], scene.tabGoursat[0].center);

    scene.nbOctaeder = 1;
    scene.tabOctaeder[0] = Octaeder(vec3(0, -6, 2.2), 1);
    scene.tabAngle[2] = vec3(0., iTime, 10.*iTime);
    scene.tabRay[2] = Rotation(scene.tabRay[2], scene.tabAngle[2], scene.tabOctaeder[0].center);
...
bool Intersect(Ray ray, inout Hit x) {
    ...
    for (int i = 0; i < scene.nbPipeConnector; i++) {
        if (IntersectPipeConnector(scene.tabRay[i + idR], scene.tabPipeConnector[i], current)) {
            if (scene.tabRay[i+idR].isHomo)
                current = Homothetie(current, ray, scene.tabRay[i+idR], scene.tabScale[i+idR]);
            if (current.t < x.t){
                x = current;
...
vec3 Color(Material m,vec3 n, vec3 p, Ray camera) {
    Scene scene;
    scene.nbLight = 2 ;
    scene.tabLight[0].lightPos = vec3(18,8,10);
    scene.tabLight[0].lightColor = vec3(1,1,1);
    scene.tabLight[1].lightPos = vec3(0,0,4);
...
void mainImage(out vec4 fragColor,in vec2 fragCoord) {
    ...
    vec3 ro=14. * normalize(vec3(sin(2. * 3.14 * mouse.x), cos(2. * 3.14 * mouse.x), 1.4 * (mouse.y - .1)));
